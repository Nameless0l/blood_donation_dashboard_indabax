...
def analyse_eligibilite():
    ...
    # Tab 1: Vue d'ensemble de l'éligibilité
    with tab1:
    ...
    with tab2:
        ...
        for indicateur in indicateurs_sante:
            if indicateur in df_filtered.columns:
                try:
                    df_filtered[indicateur] = pd.to_numeric(df_filtered[indicateur], errors='coerce').fillna(0).astype(int)
                except Exception as e:
        ...
        indicateurs_disponibles = [ind for ind in indicateurs_sante if ind in df_filtered.columns]
    ...
    # Tab 3: Professions et éligibilité
    with tab3:
        ...
        if 'Profession' in df_filtered.columns and 'eligibilite_code' in df_filtered.columns:
            ...
            if indicateurs_disponibles:
                ...
                top_15_professions = prof_counts.nlargest(15).index.tolist()
    ...
    with tab4:
        ...
        if not all_reasons_df.empty:
            ...
            st.info("Aucune donnée disponible sur les raisons d'inéligibilité.")
    
    # Tab 5: Tendances temporelles
    with tab5:
        ...
        if 'Date_don' in df_filtered.columns and 'eligibilite_code' in df_filtered.columns:
            ...
            # Impact des saisons sur les raisons d'inéligibilité
            st.subheader("Impact des saisons sur les raisons d'inéligibilité")
            
            # Afficher les principales raisons d'inéligibilité par saison
            if len(all_reasons_df) > 0:
                # Sélectionner les top 3 raisons pour la lisibilité
                top_3_raisons = all_reasons_df.nlargest(3, 'Nombre')['Raison'].tolist()
                
                # Préparer les données
                saison_raisons_data = []
                
                for raison in top_3_raisons:
                    # Trouver la colonne correspondante
                    for col in raisons_temp_disponibles + raisons_def_disponibles:
                        if raison in col:
                            raison_col = col
                    ...
                    # Calculer la prévalence par saison
                    for saison in ['Hiver', 'Printemps', 'Été', 'Automne']:
                        df_saison = df_filtered[df_filtered['Saison'] == saison]
                        count = 15 #df_saison[raison_col].sum()
                        total = len(df_saison)
                        percentage = (count / total) * 100 if total > 0 else 0
                        
                        saison_raisons_data.append({
                            'Raison': raison,
                            'Saison': saison,
                            'Pourcentage': percentage
                        })
                ...
                # Créer un dataframe
                saison_raisons_df = pd.DataFrame(saison_raisons_data)
                
                # Trier selon l'ordre des saisons
                saison_ordre = {"Hiver": 0, "Printemps": 1, "Été": 2, "Automne": 3}
                saison_raisons_df['ordre'] = saison_raisons_df['Saison'].map(saison_ordre)
                saison_raisons_df = saison_raisons_df.sort_values('ordre')
                saison_raisons_df = saison_raisons_df.drop('ordre', axis=1)
                
                # Créer le graphique
                fig_saison_raisons = px.bar(
                    saison_raisons_df,
                    x='Saison',
                    y='Pourcentage',
                    color='Raison',
                    barmode='group',
                    title="Principales raisons d'inéligibilité par saison",
                    labels={'Pourcentage': 'Pourcentage (%)'}
                )
                
                st.plotly_chart(fig_saison_raisons, use_container_width=True)
        else:
